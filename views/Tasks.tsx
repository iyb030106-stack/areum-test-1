...
const Tasks: React.FC = () => {
  ...
  // 나의 업무 - 로컬 스토리지 연동 (사용자별 격리)
  const [personalTasks, setPersonalTasks] = useState<string[]>([]);
  const uid = auth.currentUser?.uid;

  useEffect(() => {
    if (!uid) return;
    const saved = localStorage.getItem(`edu_harmony_personal_tasks_${uid}`);
    setPersonalTasks(saved ? JSON.parse(saved) : ['출결 오류 학생 전화하기', '자체 교재 제본 10권']);
  }, [uid]);

  useEffect(() => {
    if (uid) localStorage.setItem(`edu_harmony_personal_tasks_${uid}`, JSON.stringify(personalTasks));
  }, [personalTasks, uid]);

  const handleComplete = (taskName: string) => {
    const savedCompleted = localStorage.getItem(`edu_harmony_completed_tasks_${uid}`);
    ...
    if (uid) localStorage.setItem(`edu_harmony_completed_tasks_${uid}`, JSON.stringify([newEntry, ...completedList]));
