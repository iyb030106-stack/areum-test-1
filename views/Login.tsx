...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden">
      {/* Background Orbs */}
      <div className="absolute top-[-10%] left-[-10%] w-[60%] aspect-square bg-blue-400/10 rounded-full blur-[100px] animate-pulse"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[60%] aspect-square bg-emerald-400/10 rounded-full blur-[100px] animate-pulse delay-1000"></div>
          ...
          <div className="bg-white/70 backdrop-blur-xl border border-white/60 p-8 rounded-[2.5rem] shadow-2xl shadow-slate-200/30">
            ...
            {view === 'selection' && (
              <>
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  >
                    <div className="size-12 rounded-2xl bg-indigo-50 flex items-center justify-center group-hover:bg-indigo-100 transition-colors">
                  ...
                  <button 
                    ...
                    className="group flex items-center gap-5 p-5 rounded-3xl bg-white/40 border border-white/80 text-slate-800 hover:bg-white/60 transition-all active:scale-[0.98] shadow-sm"
                  >
                    <div className="size-12 rounded-2xl bg-blue-50 flex items-center justify-center group-hover:bg-blue-100 transition-colors">
            ...
            {view === 'login' && (
              <form onSubmit={handleLoginSubmit} className="space-y-5">
                <div className="flex items-center gap-2 mb-6">
                  <button type="button" onClick={() => setView('selection')} className="text-slate-400 hover:text-slate-600">
                ...
                <div className="flex justify-center pt-2">
                  <button type="button" onClick={() => setView('signup')} className="text-xs font-bold text-slate-400 hover:text-primary transition-colors">
                    신규 계정 생성하기
                  </button>
                </div>
              </form>
            ...
            {view === 'signup' && (
              <form onSubmit={handleSignupSubmit} className="space-y-5">
                <div className="flex items-center gap-2 mb-6">
                  <button type="button" onClick={() => setView('login')} className="text-slate-400 hover:text-slate-600">
                    <span className="material-symbols-outlined">arrow_back</span>
                  </button>
                  <h2 className="text-lg font-black text-slate-800 tracking-tight">회원가입</h2>
                </div>
                ...
                <div className="space-y-4">
                  <input 
                    ...
                    className="w-full px-5 py-4 bg-white/50 border border-white rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"
                  />
                  <input 
                    type="text" 
                    placeholder="아이디"
