...
const Home: React.FC<HomeProps> = ({ role }) => {
  ...
  const [lastReadNoti, setLastReadNoti] = useState<number>(0);

  useEffect(() => {
    const uid = auth.currentUser?.uid;
    if (uid) {
      const saved = localStorage.getItem(`lastReadNotiTime_${uid}`);
      setLastReadNoti(saved ? parseInt(saved, 10) : 0);
    }
  }, []);
  ...
  const handleOpenNoti = () => {
    ...
    const uid = auth.currentUser?.uid;
    setLastReadNoti(now);
    if (uid) localStorage.setItem(`lastReadNotiTime_${uid}`, now.toString());
  };
  ...
  useEffect(() => {
    ...
    const unsubNotis = subscribeToNotifications(academyId, setNotifications);

    if (role === 'admin') {
      initializeCategoriesIfNeeded(academyId, MANUAL_CATEGORIES);
    }
      ...
      <header className="px-6 pt-14 pb-2 flex items-center justify-between relative z-10">
        ...
        {role === 'admin' && (
          <button
            ...
            className={`size-10 rounded-full flex items-center justify-center shadow-sm border transition-all active:scale-90 ${isManaging ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white/45 backdrop-blur-md border-white/40 text-slate-400'}`}
          >
            <span className="material-symbols-outlined text-[20px]">{isManaging ? 'check' : 'edit'}</span>
          </button>
        )}
      </header>
        ...
        {searchQuery ? (
          <div className="bg-white/70 backdrop-blur-xl dark:bg-slate-900/70 rounded-[2.5rem] p-7 shadow-2xl border border-white/40 animate-slide-up">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setSearchQuery('')} className="text-xs font-bold text-primary active:scale-95 transition-all">닫기</button>
            </div>
          ...
          <div className="space-y-12 animate-fade-in">
            <section>
              {(showAddForm || editingCatId) && role === 'admin' && (
                      ...
                      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 px-1">
                        {CATEGORY_ICONS.map(iconName => (
                          <button
                            ...
                            className={`size-10 rounded-xl flex items-center justify-center shrink-0 transition-all ${newCatIcon === iconName ? 'bg-primary text-white scale-110 shadow-md' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'}`}
                          >
              ...
              <div className="grid grid-cols-2 gap-5 pb-10">
                ...
                {isManaging && role === 'admin' && !showAddForm && !editingCatId && (
                  <div
                    onClick={() => setShowAddForm(true)}
                    className="h-full min-h-[140px] bg-slate-50/50 dark:bg-slate-900/30 border-2 border-dashed border-slate-200 dark:border-slate-800 py-4 px-3 rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer active:scale-95 transition-all text-slate-400 hover:text-primary hover:border-primary/50"
                  >
                    <span className="material-symbols-outlined text-3xl">add_circle</span>
                    <span className="text-[10px] font-black uppercase tracking-widest">새 카테고리</span>
                  </div>
                )}
              </div>
            </section>
